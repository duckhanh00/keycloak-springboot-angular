...
public class KeycloakService {
        ...
        int statusId = 0;
         try {
             ...
             if(statusId == 201){
                 ...
                 message.setMessage("User exist");
             }
             else{
                 message.setMessage("Error, what is this?");
             }
         }catch (Exception e){
             e.printStackTrace();
         }
    ...
    private RealmResource getRealmResource(){
        Keycloak kc = KeycloakBuilder.builder().serverUrl(server_url).realm("master").username("khomachoidc")
                .password("khomachoidc").clientId("admin-cli").resteasyClient(new ResteasyClientBuilder().connectionPoolSize(10).build())
                .build();
